...
// Se ejecuta cuando se actualiza una orden existente en Shopify
export const action = async ({ request }: ActionFunctionArgs) => {
  const { topic, shop, payload } = await authenticate.webhook(request);

  console.log(`[Webhook] ${topic} recibido de ${shop}`);

  // Enviar la orden actualizada a NCF Manager para sincronización
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.log(`[Webhook] Orden ${payload.name} actualizada en NCF Manager`);
    }
  } catch (error) {
